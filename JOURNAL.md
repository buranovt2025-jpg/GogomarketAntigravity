...
## 📅 Сессия: 27 Февраля 2026 — Регистрация и База данных (Phase 10 Начало)
**Статус:** ⏳ В процессе E2E тестирования

### 🛠 E2E Тестирование и Выявленные Проблемы
- Начата **Фаза 10** (E2E Testing & Validation).
- Созданы базовые bash-скрипты для автоматической регистрации тестовых ролей (`BUYER`, `SELLER`, `COURIER`, `ADMIN`).
- Выявлена ошибка `500 Internal Server Error` на удаленном сервере (DigitalOcean) при попытке зарегистрировать пользователей.
- Причиной оказался баг сборки (Webpack) с TypeORM: Entity-классы не загружались автоматически.
- Ограничения с удаленным доступом (`SSH`) привели к переключению на локальное тестирование и отладку бэкенда через `SQLite`.

### ✅ Что сделано:
- **Backend (TypeORM)**: Реализован паттерн `autoLoadEntities: true` в `app.module.ts`, что устранило проблему `EntityMetadataNotFoundError`. Ошибка исправлена и залита в `master`.
- **Backend (Конфигурация)**: Настроен `USE_SQLITE=true` для запуска локальной in-memory/file базы данных (db.sqlite) в режиме разработки. 
- **Backend (API)**: Добавлено явное логирование `try-catch` в `register` контроллер для перехвата stack trace на фронте без логов сервера.
- **Инфраструктура**: Обновлен `create_test_accounts.sh` с переключением ссылок на API.

### 📋 Следующие шаги:
1. Завершить успешную генерацию 4-х тестовых ролей через локальный бэкенд на SQLite или обновленный DO-сервер.
2. Пройти полный цикл E2E: регистрация продавца -> создание товара -> регистрация покупателя -> заказ -> регистрация курьера -> доставка заказа -> уведомления в чате.
